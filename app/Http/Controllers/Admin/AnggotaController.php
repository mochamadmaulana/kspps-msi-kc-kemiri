...
class AnggotaController extends Controller
{
    public function index()
    {
        ...
        $draft = RegistrasiAnggota::where('status','Draft')->get()->count();
        return view('admin.anggota-new.index', compact('anggota','draft'));
    }

    public function create()
    {
        $last_no_regist = RegistrasiAnggota::orderBy('id','DESC')->where('kantor_id',Auth::user()->kantor_id)->first();
        $create_no_regist = Helpers::nomor_registrasi_anggota($last_no_regist->nomor_registrasi ?? 0,Auth::user()->kantor_id);
        $registrasi = RegistrasiAnggota::create([
            'nomor_registrasi' => $create_no_regist,
            'kantor_id' => Auth::user()->kantor_id,
            'karyawan_id' => Auth::user()->id,
        ]);
        return redirect()->route('admin.anggota.registrasi',$registrasi->id)->with('success','Berhasil membuat nomor registrasi');
    }

    public function registrasi(String $id_regist)
    {
        $data_regist = RegistrasiAnggota::findOrFail($id_regist);
        $identitas = Anggota::where('registrasi_anggota_id',$id_regist)->first();
        $alamat_identitas = NULL;
        if($identitas != NULL){
            $alamat_identitas = AlamatAnggota::where('anggota_id',$identitas->id)->first();
        }
        $anggota = Anggota::with('registrasi','alamat')->where('registrasi_anggota_id',$id_regist)->first();
        return view('admin.anggota-new.registrasi', compact('data_regist','identitas','anggota','alamat_identitas'));
    }

    public function identitas(String $id_regist)
    {
        ...
        $data_regist = RegistrasiAnggota::findOrFail($id_regist);
        return view('admin.anggota-new.identitas.create', compact('tempat_lahir', 'majlis', 'provinsi','data_regist'));
    }

    public function identitas_store(Request $request, String $id_regist)
    {
        ...
        Anggota::create([
            "uuid" => uniqid(),
            "registrasi_anggota_id" => $id_regist,
            "kantor_id" => Auth::user()->kantor_id,
            "karyawan_id" => Auth::user()->id,
        ...
        return redirect()->route('admin.anggota.registrasi',$id_regist)->with('success','IDENTITAS KTP/SIM berhasil disimpan');
    }

    public function identitas_edit(Request $request, String $id_regist, String $id)
    {
        $provinsi = Provinsi::orderBy('nama_provinsi')->get();
        $tempat_lahir = Kota::orderBy('nama_kota')->get();
        $majlis = Majlis::where('kantor_id', Auth::user()->kantor_id)->latest()->get();
        ...
        $identitas = Anggota::where('registrasi_anggota_id',$id_regist)->first();
        $alamat_identitas = AlamatAnggota::where('anggota_id',$identitas->id)->first();
        return view('admin.anggota-new.identitas.edit', compact('tempat_lahir', 'majlis', 'provinsi','jenis_usaha','data_regist','identitas','alamat_identitas'));
    }

    public function alamat_identitas(String $id_regist)
    {
        $provinsi = Provinsi::orderBy('nama_provinsi')->get();
        $data_regist = RegistrasiAnggota::findOrFail($id_regist);
        $identitas = Anggota::where('registrasi_anggota_id',$id_regist)->first();
        if($identitas != NULL){
            return view('admin.anggota-new.alamat.create', compact('provinsi','data_regist','identitas'));
        }else{
            return redirect()->route('admin.anggota.registrasi',$id_regist)->with('error','Harap lengkapi tahap ke-1 terlebih dahulu!');
        }
    }

    public function alamat_identitas_store(Request $request, String $id_regist)
    {
        $validator = Validator::make($request->all(), [
            'alamat' => ['required'],
        ...
        $anggota = Anggota::where('registrasi_anggota_id',$id_regist)->first();
        ...
        return redirect()->route('admin.anggota.registrasi',$id_regist)->with('success','ALAMAT IDENTITAS berhasil disimpan');
    }

    public function usaha(String $id_regist)
    {
        $data_regist = RegistrasiAnggota::findOrFail($id_regist);
        $jenis_usaha = JenisUsaha::get();
        $identitas = Anggota::where('registrasi_anggota_id',$id_regist)->first();
        // if($identitas != NULL){
            return view('admin.anggota-new.usaha.create', compact('data_regist','identitas','jenis_usaha'));
        // }else{
        //     return redirect()->route('admin.anggota.registrasi',$id_regist)->with('error','Harap lengkapi tahap ke-1 & 2 terlebih dahulu!');
        // }
    }
    ...
    public function store(Request $request, String $id_regist)
    {
        $validator = Validator::make($request->all(), [
            'majlis' => ['required'],
            'nomor_identitas' => ['required', 'max:17'],
