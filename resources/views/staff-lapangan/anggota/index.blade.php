@extends('layout.staff_lapangan',['title_satu'=>'List','title_dua'=>'Anggota'])

@push('btn-page-header')
<div class="col-auto ms-auto d-print-none">
    <div class="btn-list">
        <a href="{{ route('staff-lapangan.anggota.create') }}" class="btn btn-primary d-none d-sm-inline-block">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M12 5l0 14" /><path d="M5 12l14 0" /></svg>
            Anggota
        </a>
        <a href="{{ route('staff-lapangan.anggota.create') }}" class="btn btn-primary d-sm-none btn-icon">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M12 5l0 14" /><path d="M5 12l14 0" /></svg>
        </a>
    </div>
</div>
            ...
            <table class="table table-vcenter card-table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="fst-italic">{{ $a->inputer->nama_lengkap }}</td>
                        ...
                        <td>
                            @if ($a->majlis->petugas_id == Auth::user()->id)
                            <div class="btn-list flex-nowrap float-end">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a class="dropdown-item" href="{{ route('staff-lapangan.anggota.show',$a->id) }}">
                                            Detail
                                        </a>
                                        @if ($a->registrasi->inputer_id == Auth::user()->id && $a->registrasi->status != 'Diterima'&& $a->registrasi->status != 'Diproses')
                                        <a class="dropdown-item" href="{{ route('staff-lapangan.anggota.edit',$a->id) }}">
                                            Edit
                                        </a>
...
function confirmDelete(id) {
    $.ajax({
        url: '{{ url('staff-lapangan/anggota') }}/' + id,
        type: 'delete',
